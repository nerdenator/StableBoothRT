...
# MODEL PATHS
LCM_Dv7_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "LCM_Dv7_MODEL_LOCATION",
    "/Users/rolando/Documents/PROJECTS/YouTube/DIYAI_WebcamDiffusion/tutorial_scripts/models/LCM_Dreamshaper_v7",
)
LCM_Dv8_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "LCM_Dv8_MODEL_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/dreamshaper-8/",
)
CONTROLNET_CANNY_LOCATION = CONFIG.get("models", {}).get(
    "CONTROLNET_CANNY_LOCATION",
    "/Users/rolando/Documents/PROJECTS/YouTube/DIYAI_WebcamDiffusion/tutorial_scripts/models/control_v11p_sd15_canny",
)

## SDXL Models
SDXLTURBO_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "SDXLTURBO_MODEL_LOCATION",
    "/Users/rolando/Documents/PROJECTS/YouTube/DIYAI_WebcamDiffusion/tutorial_scripts/models/sdxl-turbo",
)
SDXLL_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "SDXLL_MODEL_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/SDXL-Lightning",
)
SDXL_BASEMODEL_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_BASEMODEL_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/stable-diffusion-xl-base-1.0",
)
SDXLL_CKPT_LOCATION = CONFIG.get("models", {}).get(
    "SDXLL_CKPT_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/SDXL-Lightning/sdxl_lightning_2step_unet.safetensors",
)
SDXL_CANNY_CONTROLNET_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_CANNY_CONTROLNET_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/controlnet-canny-sdxl-1.0",
)

## SDXS
SDXS_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "SDXS_MODEL_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/sdxs-512-dreamshaper/",
)
CONTROLNET_SKETCH_LOCATION = CONFIG.get("models", {}).get(
    "CONTROLNET_SKETCH_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/sdxs-512-dreamshaper-sketch/",
)

## Hyper-SD
HYPERSD_MODEL_LOCATION = CONFIG.get("models", {}).get(
    "HYPERSD_MODEL_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/Hyper-SD",
)
HYPERSD_UNET_LOCATION = CONFIG.get("models", {}).get(
    "HYPERSD_UNET_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/Hyper-SD/Hyper-SDXL-1step-Unet.safetensors",
)
HYPERSD_LORA_LOCATION = CONFIG.get("models", {}).get(
    "HYPERSD_LORA_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/models/Hyper-SD/Hyper-SDXL-1step-lora.safetensors",
)

# SDXL
SDXL_LCM_LORA_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_LCM_LORA_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/lcm-lora-sdxl",
)
SDXL_PAPERCUT_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_PAPERCUT_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/Papercut_SDXL",
)
SDXL_MIDJOURNEYV16_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_MIDJOURNEYV16_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/Midjourney-V6.1",
)
SDXL_AAM_XL_ANIMEMIX_LOCATION = CONFIG.get("models", {}).get(
    "SDXL_AAM_XL_ANIMEMIX_LOCATION",
    "/Volumes/980ProGyrus/Projects/BuildStuff2024_RTSD/AAM_XL_AnimeMix",
)
...
IMAGE_GALLERY_LOCATION = CONFIG.get("paths", {}).get(
...
def choose_device(torch_device=None):
    ...
    print(
        "...Is CUDA available in your computer?",
        "\n... Yes!" if torch.cuda.is_available() else "\n... No D': ",
    )
    print(
        "...Is MPS available in your computer?",
...
def flush():
    ...
    if torch.cuda.is_available():
        ...
        print(
            "...Couldn't flush torch cache; neither CUDA nor MPS is available on this system."
        )
...
def refresh_gallery():
    ...
    for filename in get_image_files(IMAGE_PREFIX, folder_path):
        ...
        img = img.resize(
            (
                int(img.width * (IMAGE_GALLERY_HEIGHT / img.height)),
...
def StartNewSessionFolder(base_path=IMAGE_GALLERY_MAIN):
    ...
    folder_pattern = re.compile(
        r"(\d{8})_Session_(\d{3})"
    )  # Regular expression to match the folder format "YearMonthDate_Session_001" --simple format
    subfolders = [
    ...
    if sorted_folders:
        ...
        latest_date_str, latest_session_str = folder_pattern.match(
            latest_folder
        ).groups()
    ...
    new_folder_name = (
        f"{today_str}_Session_{new_session:03d}"  # Create the new folder name
    )
    ...
    IMAGE_GALLERY_LOCATION = (
        new_folder_path  # set global variable of image gallery location to new.
    )
...
def navigateGalleryFolders(previous_or_next):
    ...
    current_session = IMAGE_GALLERY_LOCATION.split("/")[
        -1
    ]  # name of folder is after last slash

    print(
        "CURRENT STUFF: ",
        current_session,
        sorted_subfolders,
        np.where(sorted_subfolders == current_session),
    )
    ...
    if previous_or_next == "Previous":
        if index_of_current_session > 0:
            IMAGE_GALLERY_LOCATION = os.path.join(
                IMAGE_GALLERY_MAIN, sorted_subfolders[index_of_current_session - 1]
            )
    elif previous_or_next == "Next":
        if (index_of_current_session >= 0) and (
            index_of_current_session < len(sorted_subfolders)
        ):
            IMAGE_GALLERY_LOCATION = os.path.join(
                IMAGE_GALLERY_MAIN, sorted_subfolders[index_of_current_session + 1]
            )
...
def capture_and_save_images():
    ...
    if PROCESSED_IMAGE is not None:
        ...
        raw_img_path = os.path.join(
            IMAGE_GALLERY_LOCATION, f"{IMAGE_PREFIX}_{len(image_files) + 1:03d}.png"
        )
        processed_img_path = os.path.join(
            IMAGE_GALLERY_LOCATION, f"{IMAGE_PREFIX}_{len(image_files) + 2:03d}.png"
        )
...
def create_photo_strip(
    ...
    paper_width_px = int(
        paper_width_mm * dpi * inch2mm
    )  # (mm * dots / inch) * (inch / mm) = dots = pixels
    paper_height_px = int(
        paper_height_mm * dpi * inch2mm
    )  # (mm * dots / inch) * (inch / mm) = dots = pixels
    square_image_width = int(
        paper_width_px / nStrips
    )  # tells me how wide the pixels I have for each imageb
    square_image_height = int(
        paper_height_px / nPhotosPerStrip
    )  # tells me how tall the images need to be to fit
    square_image_size = np.min(
        [square_image_width, square_image_height]
    )  # the minimum is usually gonna be based on how many can be fit vertically (when trying to maximize full sets/vertical strips to print)
    scissor_padding = (
        (paper_width_px - (square_image_size * nStrips)) / (nStrips - 1)
    )  # how many square images fit with max image size to fit 3 vertically, then identify how the spacing available for cutting with scissors (nStrips-1)
    ...
    strip_height = (
        square_image_size * nPhotosPerStrip
    )  # (width, height) --> where width==height --> (width, width*3) for 3 photos per strip
    ...
    for i, img in enumerate(resized_images):
        vertical_strip.paste(
            img, (0, (square_image_size) * i)
        )  # . paste in upper left corner, then paste again at height, then height*2, etc
...
def change_parameters(model):
    ...
    NEW_PARAMETER_LIST = convert_parameters_to_gradio_list(
        PARAMETER_CONFIG, DYNAMIC_PARAMETERS
    )
...
def process_canny(image, lower_threshold=100, upper_threshold=100, aperture=3):
    ...
    )  # convert to 3 channel image (for color)
    return image
...
def resize_frame(frame, zoom_out):
    ...
    new_h, new_w = (
        h // zoom_out,
        w // zoom_out,
    )  # zoom_out tells us how much we want to zoom-out of the captured frame from camera
...
def prepare_pipeline(model):
    ...
    if model == "Dreamshaper_v7_LCM_Canny":
        controlnet = ControlNetModel.from_pretrained(
            CONTROLNET_CANNY_LOCATION, torch_dtype=TORCH_DTYPE, use_safetensors=True
        )
        ...
        pipeline = AutoPipelineForImage2Image.from_pretrained(
            LCM_Dv7_MODEL_LOCATION, torch_dtype=TORCH_DTYPE, safety_checker=None
        ).to(TORCH_DEVICE)
        ...
        pipeline = AutoPipelineForImage2Image.from_pretrained(
            LCM_Dv8_MODEL_LOCATION, torch_dtype=TORCH_DTYPE, safety_checker=None
        ).to(TORCH_DEVICE)
        ...
        controlnet = ControlNetModel.from_pretrained(
            SDXL_CANNY_CONTROLNET_LOCATION, torch_dtype=TORCH_DTYPE
        )

        unet = UNet2DConditionModel.from_config(
            SDXL_BASEMODEL_LOCATION, subfolder="unet"
        ...
        controlnet = ControlNetModel.from_pretrained(
            CONTROLNET_SKETCH_LOCATION, torch_dtype=TORCH_DTYPE
        )
        pipeline = StableDiffusionControlNetPipeline.from_pretrained(
            SDXS_MODEL_LOCATION,
            controlnet=controlnet,
        ...
        if use_adapter:
            ...
            pipeline.set_adapters(
                ["lcm", adapter_name], adapter_weights=[1.0, 1.0]
            )  # Combine LoRAs
            pipeline.fuse_lora(
                adapter_names=["lcm", adapter_name], lora_scale=1.0
...
def process_image(
    ...
    full_size_image, cropped_image = get_result_and_mask(
        input_img,
        center_x,
        center_y,
        PARAMETER_STATE["WIDTH"],
        PARAMETER_STATE["HEIGHT"],
    )

    # This function should process the image and return the output image
    if model_selected == "canny":
        image = process_canny(
            cropped_image, canny_lower_threshold, canny_upper_threshold, canny_aperture
        )
...
def list_printers():
    ...
    if system == "Windows":
        try:
            ...
            print(
                "win32print module not found. Installing pywin32... (you may need to restart app.py)"
            )
...
def print_file(selected_printer):
    ...
    fnum = len(
        get_image_files(OUTPUT_PREFIX, SAVE_FOLDER)
    )  # get most recent image number
    file_path = os.path.join(
        SAVE_FOLDER, f"output_{fnum:03}.png"
